    ...
    try {
        ...
        const pagamento = await asaas('/payments', {
            ...
            body: JSON.stringify({
                ...
                description: `Rifa do Gêmeos — ${qtd} cota(s)`
            })
        });
        ...
        res.status(500).json({
            error: erro.message || 'Erro ao criar pagamento.'
        });
    }
});
...
app.post('/api/webhook-asaas', async (req, res) => {
    try {
        ...
        if (
            ...
            venda.status = payment.status || 'RECEIVED';
...
app.get('/api/status/:id', async (req, res) => {
    ...
    try {
        ...
        venda.status = pagamento.status || venda.status;
